...
import com.networknt.petstore.service.PetServiceImpl;
...
public class PetsGetHandler implements HttpHandler {
    ...
    protected ResponseEntity listPets() throws ApiException {
        if(logger.isDebugEnabled()) logger.debug("input values:" );
        HeaderMap responseHeaders = new HeaderMap();
        responseHeaders.add(Headers.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
        ResponseEntity<String> responseEntity ;
        System.out.println("Get list pets");
        try {
...
//        Integer limit = Integer.valueOf(exchange.getQueryParameters().get("limit").getFirst());
        ...
        ResponseEntity<String> responseEntity = listPets();
